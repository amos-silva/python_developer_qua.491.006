    ...
    match opcao:    # match case = OPÇÕES
        ...
        case "2":
            try:
                if not novo_arquivo:
                    novo_arquivo = input("Informe o nome do arquivo: ").strip().lower()
                    diretorio = input("Informe o diretorio onde o arquivo está salvo: ").strip()
                with open(f"{diretorio}{novo_arquivo}.json", "r", encoding="utf-8") as t:
                    lista = json.load(t)
                os.system('cls' if os.name == 'nt' else 'clear')
                print("Arquivo JSON carregado com sucesso!")
            except Exception as e:
                print(f"Não foi possivel carregar o arquivo json: {e}")
            continue
